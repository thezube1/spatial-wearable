...
Firmware reference: `arduino/14_wearable_peer_location/14_wearable_peer_location.ino`. Firmware 13 adds a target-select characteristic (`…abd2`) so the iOS Group tab can steer the wearable at a specific group member's linked device MAC; target is persisted in NVS and filters ESP-NOW + BLE scans. Firmware 14 adds a peer-location notify characteristic (`…abd3`) that forwards the tracked target's GPS (from ESP-NOW) to iOS so the map can render both the owner and the peer. Firmware 16 (`arduino/16_wearable_group_targets/`) adds a candidate-list characteristic (`…abd4`) and a selected-target notify (`…abd5`) so iOS can push up to 8 group members for the wearer to cycle through on the NAV screen with quick taps and confirm with a 3-second hold; the wearable's selection is mirrored back to iOS through `…abd5`. Firmware 18 (`arduino/18_wearable_gps_sync/`) adds a GPS-sync characteristic (`…abd6`) so the iOS Map tab can ask the wearable to drop all radios for 30 s and force a fresh GPS fix when the on-band BLE/WiFi/ESP-NOW noise is preventing convergence; the result (success/failed) is notified back through the same characteristic. Protocol details in `documentation/ble-pairing.md` under "Target Selection", "Peer Location", "Candidate List", "Selected Target", "Target Select Sub-Mode", and "GPS Sync".
