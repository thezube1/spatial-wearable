...
// Fork of 13_wearable_target_select. Adds a second BLE notify characteristic
// (…abd3) that streams the currently-tracked target's GPS to iOS, so the
// phone's map can render the target peer alongside the owner's wearable.
//   - Payload is the same shape as own-location (abd1):
//       [valid(u8), lat(float32 LE), lon(float32 LE)]
//     where valid=0 means "no target" or "target has no GPS fix".
//   - Notifies every 3s, or immediately when the target changes/clears so
//     the phone promptly drops the pin.
//
