// Full Spatial Wearable — GPS + BLE Proximity + Heart Rate + Haptic Feedback + MAC Pairing
// Flash the SAME sketch to both XIAO ESP32S3 boards.
//
// Derived from 08_full_wearable.ino. Adds a read-only BLE characteristic
// exposing the device's STA MAC address as an ASCII string "AA:BB:CC:DD:EE:FF"
// so the companion iOS app can pair a user to a specific wearable.
//
// ============================================================
//  COMPONENTS
// ============================================================
...
void startBLE() {
  ...
  // Read-only MAC characteristic for iOS pairing.
  // Value is the 17-byte ASCII string "AA:BB:CC:DD:EE:FF" derived from
  // the STA MAC already fetched via esp_wifi_get_mac(WIFI_IF_STA, myMac).
